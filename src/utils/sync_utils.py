...
async def handle_item(get_items, item_type_orm, item_id_key, prepare_func):
    ...
    async with sessionmanager.session() as db_session:
        for item in items['items']:
            await upsertItemWithDependencies(db_session, item_type_orm, item_id_key, item, prepare_func)

        await db_session.commit()


async def upsertItemWithDependencies(db_session, item_type_orm, item_id_key, item, prepare_func):
